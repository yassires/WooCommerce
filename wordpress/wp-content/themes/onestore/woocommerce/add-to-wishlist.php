...
global $product;
if ( onestore_in_block_item() ) {
	$link_classes .= ' button icon-only';
	$fragment_options['in_block_item'] = true;
	$fragment_options['link_classes'] = ' button icon-only';
}
?>
	...
	<?php if ( ! $ajax_loading ) : ?>
		...
		<?php else : ?>
			...
			<div class="yith-wcwl-add-button">
				<a href="<?php echo esc_url( $login_url ); ?>" rel="nofollow" class="aaaaa disabled_item <?php echo esc_attr( str_replace( array( 'add_to_wishlist', 'single_add_to_wishlist' ), '', $link_classes ) ); ?>" >
