...
if ( ! function_exists( 'onestore_entry_meta_element' ) ) :
	 ...
	 */
	function onestore_entry_meta_element( $element, $classs = '' ) {
		switch ( $element ) {
			case 'date':
				$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
				if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
					$time_string = '<time class="' . $classs . ' entry-date published" datetime="%1$s">%2$s</time><time class="updated screen-reader-text" datetime="%3$s">%4$s</time>';
				}
				$time_string = sprintf(
					$time_string,
				...
				echo '<span class="' . $classs . ' entry-meta-date"><a href="' . esc_url( get_permalink() ) . '" class="posted-on">' . $time_string . '</a></span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				break;

			case 'author':
				echo '<span class="' . $classs . ' entry-meta-author-avatar author d-iflex d-v-center"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . get_avatar( get_the_author_meta( 'ID' ), apply_filters( 'onestore/frontend/meta_avatar_size', 80 ) ) . esc_html( get_the_author_meta( 'display_name' ) ) . '</a></span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				break;

			case 'categories':
				echo '<span class="' . $classs . ' entry-meta-categories cat-links">' . get_the_category_list( esc_html_x( ', ', 'terms list separator', 'onestore' ) ) . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				break;

			case 'tags':
				echo ( '<span class="' . $classs . ' entry-meta-tags tags-links">' . get_the_tag_list( '', esc_html_x( ', ', 'terms list separator', 'onestore' ) ) . '</span>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				break;

			case 'comments':
				if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					echo '<span class="' . $classs . ' entry-meta-comments comments-link">';
					comments_popup_link();
					echo '</span>';
				}
				break;
...
function onestore_header_element__hotline() {
	onestore_header_element( 'hotline' );
}
