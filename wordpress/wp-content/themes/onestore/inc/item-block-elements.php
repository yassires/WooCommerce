...
// For Product Post.
// -------------------
function onestore_item_block_post_thumb() {
	$size = 'post-thumbnail';
...
function onestore_item_block_post_title() {
	$link = get_the_permalink();
	the_title( '<h2 class="post-title item-title"><a href="' . esc_url( $link ) . '" class="item-link">', '</a></h2>' );
}

function onestore_custom_excerpt_length( $length ) {
	$custom = onestore_get_theme_mod( 'post_index_excerpt_length', 20 );
	if ( '' != $custom ) {
...
add_filter( 'excerpt_length', 'onestore_custom_excerpt_length', 999 );


function onestore_item_block_post_excerpt() {
	the_excerpt();
}

function onestore_item_block_post_cats() {
...
function onestore_item_block_post_meta() {
	$meta_elements = onestore_get_theme_mod( 'post_index_meta_elements' );
	if ( ! empty( $meta_elements ) ) {
		echo '<div class="meta-elements d-iflex d-v-center">';
		foreach ( $meta_elements as $k ) {
			onestore_entry_meta_element( $k );
		}
		echo '</div>';
	}
}

function onestore_item_block_post_link() {
	$link = get_the_permalink();
	echo '<a href="' . esc_url( $link ) . '" class="text-nowrap item-read-more-link button">' . esc_html__( 'Read more', 'onestore' ) . '</a>';
}
...
function onestore_item_block_wc_title() {
	...
	$trim  = onestore_get_theme_mod( 'woocommerce_trim_title', 1 );
	$class = 'woocommerce-loop-product__title';
	if ( $trim ) {
		$class .= ' text-trim';
	}
	echo '<h2 class="' . esc_attr( apply_filters( 'woocommerce_product_loop_title_classes', $class ) ) . '">' . get_the_title() . '</h2>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
function onestore_item_block_wc_wishlist_icon() {
	...
	echo do_shortcode( '[yith_wcwl_add_to_wishlist link_classes="wishlist-button icon-only button loop-wishlist-button" label="" product_added_text="" already_in_wishslist_text="" browse_wishlist_text=""]' ); // WPCS: XSS ok.
}
...
function onestore_item_block_wc_add_to_cart_icon() {
	...
	if ( $product ) {
		...
		echo apply_filters(
			'onestore/woocommerce_loop_add_to_cart_icon', // WPCS: XSS ok.
			sprintf(
