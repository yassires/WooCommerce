<?php
class OneStore_Sites_Placeholder {
	...
	private function init() {
		if ( get_option( 'onestore_import_placeholder_only' ) ) {
	...
	private function upload_placeholder_attachment( $name = '', $parent_id = 0 ) {
		...
		$filename = ONESTORE_SITES_PATH . $this->get_placeholder_img();
	...
	function progress_post_meta( $meta_key = false, $meta_value = false, $post_id = false ) {
		...
		switch ( $meta_key ) {
			...
			case '_product_image_gallery':
				...
				break;
			case '_onestore_page_header_image':
	...
	function progress_meta( $meta ) {
		 ...
		 *
		 * _onestore_page_header_image: array( 'id' => '', 'url' )
		 */
