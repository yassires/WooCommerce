...
class OneStore_Sites_Export {
	function __construct( $args ) {
		...
		if ( $_GET['from_onestore'] == 'placeholder' ) {
			/**
			 * @see export_wp
			 */
			update_option( 'onestore_import_placeholder_only', true );
			$this->export_wp( $args );
			die();
        } else {
			update_option( 'onestore_import_placeholder_only', null );
	...
	function the_content_export( $content ){
        return OneStore_Sites_Placeholder::get_instance()->content_replace_placeholder( $content );
    }

	function wp_get_attachment_url( $url, $post_id = false ){
		return OneStore_Sites_Placeholder::get_instance()->wp_get_attachment_url( $url, $post_id );
    }
	...
	function export_wp( $args = array() ) {
		...
		>
			...
			<channel>
				 ...
				 */
                if ( OneStore_Sites_Placeholder::get_instance()->placeholder_post ) {
	                $post = OneStore_Sites_Placeholder::get_instance()->placeholder_post;
	                setup_postdata( $post );
	                $this->post_item( $post );
                }

				if ( OneStore_Sites_Placeholder::get_instance()->logo_post ) {
					$post = OneStore_Sites_Placeholder::get_instance()->logo_post;
					setup_postdata( $post );
					$this->post_item( $post );
				}
	...
	function progress_elementor_data( $data ){
	    return OneStore_Sites_Placeholder::get_instance()->progress_elementor_data( $data );
    }

	function progress_meta( $meta ){
        return OneStore_Sites_Placeholder::get_instance()->progress_meta( $meta );
    }
...
function onestore_sites_export_wp( $args = array() ){
	if ( isset( $_GET['download'], $_GET['content'], $_GET['from_onestore'] ) ) {
		new OneStore_Sites_Export( $args );
	}
}
add_action( 'export_wp', 'onestore_sites_export_wp' );