...
(function ($) {
    ...
    $(function () {
        ...
        $body.on(
            "click",
            ".onestore-admin-upload-control-button",
            function (e) {
                e.preventDefault();

                var $button = $(this),
                    $control = $button.closest(
                        ".onestore-admin-upload-control"
                    ),
        ...
        $body.on("change", ".onestore-admin-dependent-field", function () {
            var $select = $(this),
                $settings = $(
                    '[data-dependency="' + $select.attr("name") + '"]'
                ),
                value = this.value;
        ...
        $(".onestore-admin-metabox").each(function () {
            ...
            $navigation.on(
                "click",
                ".onestore-admin-metabox-nav-item a",
                function (e) {
                    e.preventDefault();
                    ...
                    if ($target && !$target.hasClass("active")) {
                        $navigation
                            .children(".onestore-admin-metabox-nav-item.active")
                            .removeClass("active");
                        $link
                            .parent(".onestore-admin-metabox-nav-item")
                            .addClass("active");

                        $panels
                            .children(".onestore-admin-metabox-panel.active")
                            .removeClass("active");
        ...
        $(".onestore-rating-notice").on(
            "click",
            ".onestore-rating-notice-close",
            function (e) {
                var $link = $(this),
        ...
        $(".onestore-admin-install-sites-import-plugin-button").on(
            "click",
            function (e) {
                var $button = $(this);
                ...
                return $.ajax({
                    ...
                    if (response.success) {
                        $button.html(
                            OneStoreAdminData.strings.redirecting_to_demo_list
                        );

                        window.location = OneStoreAdminData.sitesImportPageURL;
                    } else {
                        alert(
                            OneStoreAdminData.strings.error_installing_plugin
                        );
                    }
                });
            }
